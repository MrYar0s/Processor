...
int StackDump(Stack* tack, int err){
    ...
    switch(prog){
        ...
        case DOUBLE_TYPE:
                            fprintf(code, "First canary = %lg\n", tack->data[0]);
                            fprintf(code, "Second canary = %lg\n", tack->data[tack->maxlen + 1]);
                            break;

        case INT_TYPE:      fprintf(code, "First canary = %d\n", tack->data[0]);
                            fprintf(code, "Second canary = %d\n", tack->data[tack->maxlen + 1]);
                            break;

        case CHAR_TYPE:     fprintf(code, "First canary = %c\n", tack->data[0]);
    ...
    for (size_t i = 1; i <= tack->maxlen; i++){
        ...
        if (tack->data[i] == POISON)
            ...
            case DOUBLE_TYPE:   fprintf(code, "[%zu]: %lg\n", i, tack->data[i]);
                                break;

            case INT_TYPE:      fprintf(code, "[%zu]: %d\n", i, tack->data[i]);
                                break;

            case CHAR_TYPE:     fprintf(code, "[%zu]: %c\n", i, tack->data[i]);
                                break;
